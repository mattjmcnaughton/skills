...
1. Capture the current working directory as an absolute path before starting a
   service. Uploads must land in `<working-directory>/.agentic/uploads`, even if
   ...
   worktree, verify with `git check-ignore .agentic/uploads/probe` from that working
...
3. Run `<runner> <absolute-skill-path>/scripts/server.py --directory <working-directory>/.agentic`
   using the environment's process supervisor, the client's background-process
   facility, or a persistent terminal.
...
5. On a local machine, share `http://localhost:<port>/#token=<token>`. For a
   remote machine, use its approved HTTPS proxy or SSH tunnel and share a URL
   reachable from the user's browser, with `#token=<token>` appended. The
   fragment supplies the upload credential without putting it in HTTP request
   URLs. Do not send sensitive uploads over plain HTTP on an untrusted network.
...
- Files land in `<working-directory>/.agentic/uploads/`, excluded from Git. List this directory
...
- Stop the server using the same process manager or terminal that started it
  when the user is done uploading. Keep received files until the task is
  finished; ask before deleting user uploads. Stopping the server does not delete
  `.agentic/uploads`. Files remain subject to the host workspace's lifetime;
